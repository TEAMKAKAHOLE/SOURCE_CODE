...
void EscapeScene::Start()
{
	...
	m_teleportB.SetBodyPos({ (int)418 * 4 ,(int)208* 4 });
	m_teleportB.SetBodySize({ (int)100,(int)100 });

	
	m_teleportC.SetBodyPos({ (int)250 * 4 ,(int)250 * 4 });
	m_teleportC.SetBodySize({ (int)100,(int)100 });

	
	m_teleportD.SetBodyPos({ (int)92 * 4 ,(int)290 * 4 });
...
void EscapeScene::Update()
{
	...
	RECT rt;
	//탈출시 위치가 또다른 충돌박스에 있지 않게 한다.
	if (IntersectRect(&rt, &m_player.GetBodyRect(), &m_teleportA.GetBodyRect()))
	{
		m_player.SetBodyPos({ (int)250 * 4,(int)250 * 4 });
	}else

	if (IntersectRect(&rt, &m_player.GetBodyRect(), &m_teleportB.GetBodyRect()))
	{
		m_player.SetBodyPos({ (int)92 * 4,(int)290 * 4 });
	}

	else
	if (IntersectRect(&rt, &m_player.GetBodyRect(), &m_teleportC.GetBodyRect()))
	{
		m_player.SetBodyPos({ (int)80* 4,(int)170 * 4 });
	}

	else
	if (IntersectRect(&rt, &m_player.GetBodyRect(), &m_teleportD.GetBodyRect()))
	{
		m_player.SetBodyPos({ (int)385 * 4,(int)300 * 4 });
	}
